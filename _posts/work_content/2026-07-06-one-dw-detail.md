...
#### 1. 도메인 기반 DW 구조 설계
* DW를 주요 업무 도메인별로 분리해 각 특성에 맞는 독립적 스키마로 설계하여 데이터 주체 명확화했습니다.
</div>

<div class="post-step" markdown="1">
#### 2. Kafka 파이프라인 구현
* Kafka에서 실시간 수집된 JSON 로그 데이터를 내장 파서를 활용하여 임시 테이블에 적재하고, 필드 누락 및 중복 제어를 위한 전처리 로직을 자동화했습니다.
...
<div class="post-step" markdown="1">
#### 3. 대용량 쿼리 튜닝
* 고비용 SQL 튜닝을 통해 로직을 리팩토링하고(`NOT IN`, `NOT EXISTS` 전환, 윈도우 함수 `MIN`/`MAX` 구조 변경), 자주 조인되는 결합 키 중심의 프로젝션(Projection) 최적화 및 정렬 순서(Order by)를 재설정 진행했습니다.
